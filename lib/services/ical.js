...
function event(mail) {
  ...
  try {
    // XXX ugly hack, override content with a string, buffers are not well supported in nedb/mongo
    if (attachment.content.buffer) {
      logger.info('Buffer found in attachment content');
    }

    var content = _.toString(attachment.content);
